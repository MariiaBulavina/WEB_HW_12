...
async def read_contacts(name: str = None, last_name: str = None, email: str = None, db: Session = Depends(get_db), current_user: User = Depends(auth_service.get_current_user)):
    
    contacts = await repository_contacts.get_contacts(db, name, last_name, email)
    return contacts


@router.get('/birthdays', response_model=List[ContactResponse])
async def read_contacts_birthdays(db: Session = Depends(get_db), current_user: User = Depends(auth_service.get_current_user)):
